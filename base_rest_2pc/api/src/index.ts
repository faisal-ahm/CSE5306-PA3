...
import { Pool } from "pg";
import { TwoPcClient } from "./two_pc_client";
...
const COORDINATOR_NODE_ID = process.env.INSTANCE_NAME || "api-coordinator";

const PARTICIPANTS = [
  { id: "participant-1", host: "participant1", port: 50051 },
  { id: "participant-2", host: "participant2", port: 50051 },
  { id: "participant-3", host: "participant3", port: 50051 },
  { id: "participant-4", host: "participant4", port: 50051 },
];

const twoPcClient = new TwoPcClient(COORDINATOR_NODE_ID, PARTICIPANTS);
...
app.post("/polls", async (c) => {
  try {
    ...
    // Run 2PC first (participants just vote, no DB)
    const result = await twoPcClient.runTwoPhaseCommit({
      txId,
    ...
    if (!result.globalCommit) {
      return c.json(
        {
          error: "Global abort from 2PC, poll not created",
          reason: result.reason || "participant voted abort",
        },
        500
      );
    }

    // Only if 2PC says COMMIT, we do the real DB insert (same as original)
    const client = await pool.connect();
    try {
