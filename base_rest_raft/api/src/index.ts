...
import { Pool } from "pg";
import { raftCreatePoll } from "./raft_client";
...
app.post("/polls", async (c) => {
  try {
    ...
    // 1) Send it to Raft cluster
    const raftResult = await raftCreatePoll(body.question, body.options);
    if (!raftResult.ok) {
      return c.json(
        {
          error: "Raft cluster did not accept operation",
          details: raftResult.message,
          leaderId: raftResult.leaderId,
        },
        500
      );
    }

    // 2) Only after Raft commits (simulated), we write to DB (leader applies op)
    const client = await pool.connect();
    try {
      ...
      return c.json(
        {
          ...
          raftLogIndex: raftResult.logIndex,
        },
        201
      );
    } finally {
    ...
    console.error("Create poll error (Raft):", error);
